...
std::vector<YOLODetection> YOLOv5Detector::postprocess(const std::vector<float>& output, const cv::Size& original_shape) {
    ...
    for (int i = 0; i < 25200; ++i) {
        ...
        detections.push_back({bbox, final_confidence, class_id, 0.0f, 0});
    }
...
private:
    ...
    void declare_parameters() {
        ...
        // 距离估计参数
        this->declare_parameter<double>("distance_scale_factor", 1000.0);  // 距离缩放因子
        this->declare_parameter<double>("reference_lightbar_length", 30.0); // 参考灯条长度（像素）
        this->declare_parameter<double>("reference_distance", 3.0);        // 参考距离（米）
        this->declare_parameter<double>("min_detection_distance", 0.5);    // 最小检测距离（米）
        this->declare_parameter<double>("max_detection_distance", 10.0);   // 最大检测距离（米）
        
        // 灯条检测参数（在装甲板区域内）
        this->declare_parameter<double>("lightbar_min_area", 10.0);
        this->declare_parameter<double>("lightbar_max_area", 1000.0);
        this->declare_parameter<double>("lightbar_min_aspect_ratio", 0.5);      // 长宽比
        this->declare_parameter<double>("lightbar_max_aspect_ratio", 80.0);     // 长宽比
        ...
        // 矩形检测参数
        this->declare_parameter<int>("rect_min_area", 500);
    ...
    // 距离估计函数 - 基于灯条平均长度
    float estimateDistanceByLightBars(const vector<LightBar>& lightBars) {
        double distance_scale_factor = this->get_parameter("distance_scale_factor").as_double();
        double reference_length = this->get_parameter("reference_lightbar_length").as_double();
        double reference_distance = this->get_parameter("reference_distance").as_double();
        double min_distance = this->get_parameter("min_detection_distance").as_double();
        double max_distance = this->get_parameter("max_detection_distance").as_double();
        
        if (lightBars.empty()) {
            return static_cast<float>(max_distance); // 没有灯条，返回最大距离
        }
        
        // 计算灯条平均长度
        float avg_length = 0.0f;
        for (const auto& lightBar : lightBars) {
            avg_length += lightBar.length;
        }
        avg_length /= lightBars.size();
        
        // 基于相似三角形原理：距离 ∝ 1/灯条长度
        // distance = (reference_distance * reference_length) / avg_length
        float distance = static_cast<float>((reference_distance * reference_length) / avg_length);
        
        // 应用缩放因子进行校准
        distance = distance * static_cast<float>(distance_scale_factor / 1000.0);
        
        // 限制距离范围
        distance = std::max(static_cast<float>(min_distance), distance);
        distance = std::min(static_cast<float>(max_distance), distance);
        
        return distance;
    }
    
    // 装甲板排序函数 - 按距离从近到远排序
    void sortArmorsByDistance(vector<DetectionResult>& armors) {
        std::sort(armors.begin(), armors.end(), 
            [](const DetectionResult& a, const DetectionResult& b) {
                return a.distance < b.distance;  // 距离小的排在前面
            });
        
        // 分配排序序号
        for (int i = 0; i < armors.size(); i++) {
            armors[i].sort_order = i + 1;  // 从1开始编号
        }
    }
...
vector<VisionNode::DetectionResult> VisionNode::detectYOLOArmors(Mat& image) {
    ...
    try {
        ...
        for (auto& det : yolo_detections) {
            ...
            // 基于灯条平均长度估计距离
            det.distance = estimateDistanceByLightBars(lightBars);
            ...
            std::string distance_text = "D:" + std::to_string(det.distance).substr(0, 4) + "m";
            std::string lightbar_text = "L:" + std::to_string(lightBars.size());
            
            int base_line;
            cv::Size label_size = cv::getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, 0.6, 2, &base_line);
            cv::Size distance_size = cv::getTextSize(distance_text, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &base_line);
            cv::Size lightbar_size = cv::getTextSize(lightbar_text, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &base_line);
            ...
            // 绘制标签背景
            int max_width = std::max({label_size.width, distance_size.width, lightbar_size.width});
            cv::rectangle(image, 
                         cv::Point(label_origin.x, label_origin.y - label_size.height - base_line),
                         cv::Point(label_origin.x + max_width, 
                                  label_origin.y + base_line + distance_size.height + lightbar_size.height + 10),
                         color, cv::FILLED);
            ...
            // 绘制距离信息
            cv::putText(image, distance_text,
                       cv::Point(label_origin.x, label_origin.y + distance_size.height + 5),
                       cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255), 1);
            
            // 绘制灯条数量信息
            cv::putText(image, lightbar_text,
                       cv::Point(label_origin.x, label_origin.y + distance_size.height + lightbar_size.height + 10),
                       cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255), 1);
            ...
            result.distance = det.distance;
            result.sort_order = 0; // 稍后排序
            ...
            RCLCPP_INFO(this->get_logger(), 
                       "YOLO detected armor %s with confidence %.2f, distance: %.2fm, lightbars: %zu", 
                       class_name.c_str(), det.confidence, det.distance, lightBars.size());
        }
        
        // 对装甲板按距离排序
        sortArmorsByDistance(results);
        
        // 在图像上显示排序序号
        for (int i = 0; i < results.size(); i++) {
            const auto& result = results[i];
            cv::Rect bbox(
                static_cast<int>(result.points[0].x),
                static_cast<int>(result.points[0].y),
                static_cast<int>(result.points[2].x - result.points[0].x),
                static_cast<int>(result.points[2].y - result.points[0].y)
            );
            
            // 在装甲板中心显示排序序号
            cv::Point center(bbox.x + bbox.width/2, bbox.y + bbox.height/2);
            std::string order_text = "#" + std::to_string(result.sort_order);
            
            cv::putText(image, order_text,
                       cv::Point(center.x - 10, center.y),
                       cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 0, 255), 3);
            cv::putText(image, order_text,
                       cv::Point(center.x - 10, center.y),
                       cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(255, 255, 255), 2);
            
            RCLCPP_INFO(this->get_logger(), 
                       "Armor %s sorted as #%d with distance %.2fm", 
                       std::to_string(result.armor_number).c_str(), result.sort_order, result.distance);
        }
...
// 对灯条顶点进行排序（从左下角开始逆时针：左下(1) -> 左上(2) -> 右上(3) -> 右下(4)）
vector<Point> VisionNode::sortLightBarVertices(const vector<LightBar>& lightBars, const cv::Rect& armor_bbox) {
    ...
    // 重新设计排序逻辑：从左下角开始逆时针
    vector<Point> sortedVertices(4);
    
    // 分类四个顶点到四个角
    for (const auto& vertex : allVertices) {
        // 左下角：x较小，y较大
        if (vertex.x <= center.x && vertex.y >= center.y) {
            sortedVertices[0] = vertex; // 左下(1)
        }
        // 右下角：x较大，y较大
        else if (vertex.x >= center.x && vertex.y >= center.y) {
            sortedVertices[1] = vertex; // 右下(2)
        }
        // 右上角：x较大，y较小
        else if (vertex.x >= center.x && vertex.y <= center.y) {
            sortedVertices[2] = vertex; // 右上(3)
        }
        // 左上角：x较小，y较小
        else if (vertex.x <= center.x && vertex.y <= center.y) {
            sortedVertices[3] = vertex; // 左上(4)
        }
    }
...
void VisionNode::drawLightBarVertices(Mat& image, const vector<LightBar>& lightBars, const cv::Rect& armor_bbox) {
    ...
    for (int i = 0; i < sortedVertices.size(); i++) {
        ...
        RCLCPP_DEBUG(this->get_logger(), "LightBar vertex %d: (%d, %d)", 
                    i + 1, sortedVertices[i].x, sortedVertices[i].y);
    }
}
